...
Run `make` to build all targets for Android (`aarch64-linux-android33-clang`).

All binaries now take `CORE` as the first argument and pin themselves to that core. PMU cycle timing uses `pmccntr_el0`, and each binary calls `try_enable_pmu_for_this_process()` before measurements.

Current usage:

* `./load-loop-timing CORE REPS RANDOM`
* `./mispredict CORE`
* `./oob-read CORE`
* `./rogue-function CORE`
* `./aliasing CORE`
...
Run it as `./load-loop-timing <core> 10 0` (memset), and you'll see something like this:
...
And then `./load-loop-timing <core> 10 1` (random fill), and the results will be vastly different:
