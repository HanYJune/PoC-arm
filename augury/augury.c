...
int main(int argc, char **argv) {
        ...
        if (argc != 6)
        {
                ...
                fprintf(stderr,
                        "Enter: %s "
                        "<num_of_train_pointers> <offs_past_train_buf> "
                        "<repetitions> "
        ...
        if (core_ID > 7 || core_ID < 0) {
                fprintf(stderr,
                        "Wrong core! core_ID should be less than %d and more "
                        "than 0!\n",
                        7);
                exit(1);
        }
        ...
        int num_of_test_pointers =
            offs_past_train_buf < 2 + 80 ? 2 + 80 : offs_past_train_buf;
        ...
        int size_of_memory_touched_in_training =
            ...
            CACHE_LINE_SIZE; // one pointer per cache line

        int size_of_memory_checked_in_testing =
            num_of_test_pointers * u64_ptrs_per_cacheline *
            CACHE_LINE_SIZE; // one pointer per cache line

        int total_memory_allocated_for_aop =
            size_of_memory_touched_in_training +
        ...
        volatile uint64_t **aop =
            mmap(0, total_memory_allocated_for_aop + MB(2),
        ...
        for (int i = 0; i < total_num_of_pointers; i += 1) {
                ...
                if (i == num_of_train_pointers - 1) {
                        printf("[+] Last access of training will be "
                               "aop[%u](%p)=%p\n",
                               i * u64_ptrs_per_cacheline,
                               &aop[i * u64_ptrs_per_cacheline],
                               aop[i * u64_ptrs_per_cacheline]);
                        /* dprint("[dbg-alloc] Reset stride from %llu ",
                         * stride); */
                        /* stride = 1; */
                        /* dprint("to %llu\n", stride); */
                }
        ...
        // Allocate a large array (8 times the cache) that we can access to
        ...
        for (uint32_t i = 0; i < repetitions * 2; i++) {
                ...
                for (uint32_t j = 0; j < num_of_train_pointers /* + 1 */; j++) {
                        __trash =
                            (rand_idx + aop_idx + j) | (__trash & MSB_MASK);

                        // Direct data access
                        // When aop_mode == 1 we always load data_buffer[16]
                        // Otherwise, when aop_mode == 0, we follow the data
                        // accesses in the order we filled the AOP
                        corresponding_idx_in_data_buffer =
                            rand_idx * (1 - aop_mode) * u64s_per_cacheline;
                        ...
                        __trash = data_buffer[corresponding_idx_in_data_buffer |
                                              (__trash & MSB_MASK)];

                        // Data access through AoP
                        // When aop_mode == 1, we always load the next pointer
                        // in the aop Otherwise, when aop_mode == 0, we always
                        // load aop[0] (which is = data_buffer[16]) The modulo
                        // operation prevents speculatively accessing data past
                        // the end of the array during training.
                        __trash = *aop[(aop_idx % num_of_train_pointers) *
                                           u64_ptrs_per_cacheline |
                                       (__trash & MSB_MASK)];

                        /* uint64_t inc = j != (num_of_train_pointers-1) ?
                         * aop_mode : 16385
                         * * */
                        /*     aop_mode; */
                ...
                /* __trash = *aop[(aop_idx + 16384) * u64_ptrs_per_cacheline |
                 * (__trash
                 * & */
                /* 		MSB_MASK)]; */
                ...
                /* int retval = nanosleep((const struct timespec[]){{0,
                 * 10000000L}}, */
                /*         NULL); */
                ...
                __trash =
                    data_buffer[test_idx_in_data_buffer | (T1 & MSB_MASK)];
                // __trash = *aop[(num_of_train_pointers + offs_past_train_buf -
                // 1) * u64_ptrs_per_cacheline | (T1 & MSB_MASK)];
                ...
                /* printf("[+] The extra access was at aop[%u] (%p) = %p -> %"
                 * PRIx64 */
                /*         "\n", (aop_idx) * u64_ptrs_per_cacheline, */
                /*         &aop[(aop_idx) * u64_ptrs_per_cacheline], */
                /*         aop[(aop_idx) * u64_ptrs_per_cacheline], */
