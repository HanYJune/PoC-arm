...
static inline __attribute__((always_inline)) uint64_t read_pmccntr_raw(void) {
	uint64_t cycles;
...
static inline uint64_t measure_load_cycles(const uint8_t *addr) {
	__asm__ __volatile__("dsb sy" ::: "memory");
	__asm__ __volatile__("isb" ::: "memory");
	uint64_t t1 = read_pmccntr_raw();
	ForceRead(addr);
	__asm__ __volatile__("dsb sy" ::: "memory");
	__asm__ __volatile__("isb" ::: "memory");
	uint64_t t2 = read_pmccntr_raw();
	return t2 - t1;
}

void try_enable_pmu_for_this_process(void);
