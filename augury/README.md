...
### Building for Android (ARM64 / Pixel)

The Makefile supports Android NDK cross-compilation for ARM64.
Example:

```bash
make ANDROID=1 API_LEVEL=33
```

If your NDK is not at `~/android-ndk-r27c`, set:

```bash
make ANDROID=1 ANDROID_NDK_HOME=/path/to/android-ndk
```

On Android builds, the default binaries are `augury` and `imp` under `bin/`.
