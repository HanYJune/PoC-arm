...
## ARM Pixel 10 (Android) Setup
This tree is now prepared for `aarch64-linux-android` builds.

### Reverse Engineering (`re`)
Use Android NDK clang (example: API 33):
```
cd re/src
make clean
make TARGET_OS=android CC=/home/hyj/android-ndk-r27c/toolchains/llvm/prebuilt/linux-x86_64/bin/aarch64-linux-android33-clang
```

Or run:
```
cd re
ANDROID_NDK_HOME=/home/hyj/android-ndk-r27c ./init.sh
```

### PoC (`poc`)
Use Rust Android target:
```
rustup target add aarch64-linux-android
cd poc/crypto_attacker
cargo build --release --target aarch64-linux-android
```
`libaugury-ffi-sys`/`libkyber-ffi-sys` no longer hardcode `aarch64-apple-darwin`.
